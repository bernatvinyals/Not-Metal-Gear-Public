...
void SceneEquipment::init()
{
	demo.CreateActor(0, 0, "./assets/img/inventory/DemoEquipmentInv.png",true, false);
	cursor.CreateActor(0, 0, "./assets/img/persistent/Selector.png", true, false);
	mReinit = false;
	maxAmount = 0;
}

void SceneEquipment::render()
{
	demo.Render();
	std::vector<AItem*>* itemlist = GameData::getInstance()->GetInventory(0);
...
void SceneEquipment::update()
{
	...
	if (Input::getInstance()->keyboard.KB_RETURN && notPressedDown)
	{
		...
		GameData::getInstance()->SetSelectedUtility(SelectedIndex);
