...
int ResourceManager::addRawGraphic(SDL_Texture* source, const char* strName)
{
	if (mFirstFreeSlot != -1)
	{
		mGraphicsVector[mFirstFreeSlot] = source;
		mIDMap.insert(
			std::pair<std::string, Sint32>(strName, mFirstFreeSlot));
	...
	{
		mGraphicsVector.push_back(source);
		...
		mIDMap.insert(
			std::pair<std::string, Sint32>(strName, mGraphicsVector.size() - 1));

		return mGraphicsVector.size() - 1;
	}
}

int ResourceManager::getGraphicByPath(const char* strName) {
	std::map<std::string, Sint32>::iterator it;
...
Sint32 ResourceManager::createGraphic(const char* name, Uint16 width, Uint16 height, Uint8 r, Uint8 g, Uint8 b)
{
	SDL_Surface* tempSurface = SDL_CreateRGBSurface(0,width,height,32,0,0,0,0);
	if (!SDL_FillRect(tempSurface, &tempSurface->clip_rect, SDL_MapRGB(tempSurface->format, r, g, b)))
	{
		if (mFirstFreeSlot != -1)
		{
